   ...
   */
   ...
   async filter(options={ verbose: false }) {
      try {
         ...
         if (options.verbose) { log(`Filter by keyword '${keyword}' finished`) }
         
         return true
      }
      catch (err) {
         log(`ERROR: Cannot find available forms - SKIP`)
         console.error(err)
         return false
      }
   }
   ...
   async display(options={ verbose: false }) {
      try {         
         ...
         if (!availables.includes(this.displayNumber)) {
            log(`ERROR: Display number ${this.displayNumber} is not in availables - SKIP`)
            return false
         }

         const dispUrl = `${config.URLs.dispUrl}${this.displayNumber}`
         await this.page.goto(dispUrl)                                             // display n forms
         if (options.verbose) { log(`Display ${this.displayNumber} forms finished`) }
   ...
   async get(options={ verbose: false }) {
      ...
      try {
         ...
         if (options.verbose) { log("Find offer forms finished. Total forms found: " + this.formList.length) }

         return true
      }
      catch (err) {
         log(`ERROR: Cannot find offer forms - SKIP`)
         console.error(err)
         return false
      }
   }
