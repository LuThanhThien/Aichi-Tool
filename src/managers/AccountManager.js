...
async function logIn(page, account, depth=0, maxDepth=10) {
   ...
   try {
      ...
      try {
         ...
         log(`ERROR: Cannot login, retry time <${depth}>...`, account)
         if (depth < maxDepth || maxDepth == 0) {
            return await logIn(page, account, depth+1)
         }
         else {
            log(`ERROR: Cannot login`, account)      
            return false
         }
      }
   } 
...
async function accountFormsInquery(page, account) {
   ...
   try {
      ...
      log(`ERROR: Cannot navigate to inquery page`, account)
      return false
   }
   ...
   log(`Found ${formListLength} forms`, account)
   return formList
}
