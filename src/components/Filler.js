...
export default async function(disPages, listForms, filledForms={}, capture=false, test=false, multiForms=false, hidden=false) {
   ...
   await Promise.all(disPages.map(async (loggedPage, pageIndex) => {
      ...
      if (filledForms[thisAccount.username].length >= 3 && test == true) {
         log(`This account has reach max forms - END`, thisAccount, false)
         return
      }
      ...
      if (multiForms == false) {
         while (totalForms > 0 && n < listForms.length) {
            ...
            if (filledForms[thisAccount.username].includes(thisForm.title)) {
               log(`Auto fill form ${thisForm.title} skipped`, thisAccount, false)
               n++
               continue
            }
            log(`Auto fill form ${thisForm.title} begin`, thisAccount)
            ...
            log(`Auto fill form ${thisForm.title} finished - ${isSuccess ? 'SUCCESS' : 'FAILED'}`, thisAccount)        
            if (!isSuccess) {
               ...
               log('Filled info: ' + JSON.stringify(thisInfo[totalForms-1]), thisAccount)
               totalForms--
               totalSuccess++
            }
            await newPage.close()
         ...
         await Promise.all(listForms.map(async (thisForm, i) => {
            if (filledForms[thisAccount.username].includes(thisForm.title)) {
               log(`Auto fill form [${n+1}] skipped: ${thisForm.title}`, thisAccount, false)
               return
            }
            log(`Auto fill form [${n+1}] begin: ${thisForm.title}`, thisAccount)
            const newPage = await thisPage.browser().newPage()
            // await newPage.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36')
            let isNavigatedToForm = await utils.navigateTo(newPage, thisForm.link)
            let isFail = await formManager.filler(newPage, thisAccount, thisForm, n+1, capture, test, thisInfo[totalForms-1])
            log(`Auto fill form [${n+1}] finished - ${isFail ? 'FAILED' : 'SUCCESS'}`, thisAccount)        
            if (!isFail) {
               const fail = {account: thisAccount.username, number: n+1, title: thisForm.title}
               failStore.push(fail)
            }          
               ...
               filledForms[thisAccount.username].push(thisForm.title)
               log('Filled info: ' + JSON.stringify(thisInfo[totalForms-1]), thisAccount, false)
               totalForms--
               totalSuccess++
            }
            await newPage.close()
      ...
      log("Account finished - END", thisAccount, false)}
      catch (err) {
         log("ERROR: Account finished - END", thisAccount)
         console.log(err)
      }
   }))
